  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse res) throws IOException {
    ...
    if (req.getParameter("code") != null) {
      ...
      TokenResponse tokenResponse =
        ...
        AuthorizationCodeTokenRequest tk = flow.newTokenRequest(req.getParameter("code"));

      // Extract the Google User ID from the ID token in the auth response
      String userId = ((GoogleTokenResponse) tokenResponse).parseIdToken().getPayload().getUserId();

        //When Dance is over, insert subscription.

        Database db = new Database();
        if(db.usrCanTweet(userId)) {
            Credential credential = AuthUtil.newAuthorizationCodeFlow().loadCredential(userId);
            MirrorClient.insertSubscription(
                    credential,
                    "https://mirrornotifications.appspot.com/forward?url=http://ec2-54-173-226-91.compute-1.amazonaws.com:8080/notify",
                    userId,
                    "timeline");
        }
